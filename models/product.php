 ...
 */
class JomdirectoryModelProduct extends JModelAdmin
{
	/**
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_jomdirectory.product.reviews.data', 'addreviews', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	public function getFormTellAFriend($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_jomdirectory.products.tellafriend.data', 'addtellafriend', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	public function getFormContact($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_jomdirectory.products.contact.data', 'addcontact', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	public function getReviews($item)
	{
		...
		$select = $this->_db->getQuery(true);
		$select->select('a.*, IF(a.user_id IS NOT NULL, b.name, a.username) AS username ');
		...
		$select->where('a.extension = "com_jomdirectory.products"');
		...
		$rated_elements = $this->getElements($listing->language, 'ratingsProducts');
		$recommended_elements = $this->getElements($listing->language, 'recommendationsProducts');
		foreach ($item AS $i) {
	...
	protected function getOldValue($review_id, $type_id, $column)
	{
		if ($review_id) {
			...
			$query->where('extension ="com_jomdirectory.products"');
			$db->setQuery($query);
	...
	public function getItem($idOn = false)
	{
		...
		$select->select('a.*, b.title AS categoryTitle');
		$select->from('#__cddir_products AS a');
