 ...
 */
class JomdirectoryModelAdmin_additem extends JModelAdmin
{
	/**
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_jomdirectory.admin_additem', 'admin_additem', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	public function save($data)
	{
		...
		if ($limits = $this->getPlanLimits()) {
			...
			echo $data['phone'];
			echo "x".$limits->paid_fields['phone'];
			if ($data['phone'] && (!isset($limits->paid_fields['phone']) || $limits->paid_fields['phone'] == 0)) {
				JError::raiseWarning(100, JText::_("COM_JOMDIRECTORY_NO_PHONE_IN_PLAN"));
				$data['phone'] = '';
			}
			if ($data['webpage'] && (!isset($limits->paid_fields['webpage']) || $limits->paid_fields['webpage'] == 0)) {
				JError::raiseWarning(100, JText::_("COM_JOMDIRECTORY_NO_WEBPAGE_IN_PLAN"));
				$data['webpage'] = '';
			}
			
			if ($listings_nr >= $limits->listings_nr && !$data['id']) {
				$this->setError(JText::_("COM_JOMDIRECTORY_NOT_ENOUGH_LISTINGS_IN_PLAN"));
				return false;
			}
		...
		if (parent::save($data)) {
			...
			$data['link'] = JURI::base() . "administrator/index.php?option=com_jomdirectory&view=item&layout=edit&id=" . $id;
			$data['user'] = $this->user->username;
			$email[0] = $config->get('mailfrom');
			if ($params_config->get('admin_notification_items') && !$params_config->get('admin_approve_items')) {
				...
				//$mailer->setBody($body);
				//$mailer->Send();
				$send = Main_Mail::send($data, 'LISTING_ADD', $email, 'com_jomdirectory');
	...
	public function getLimit()
	{
		...
		if ($limits = $this->getPlanLimits()) {
			if ($listings_nr >= $limits->listings_nr && !$item->id) {
				$resp[0] = true;
			}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_jomdirectory.edit.admin_additem.data', array());
		...
		if ($item = parent::getItem($pk)) {
			...
			if ($jVerArr[0] >= '3') {
				...
				if (!empty($item->id)) {
					...
					$item->tags->getTagIds($item->id, 'com_jomdirectory.content');
