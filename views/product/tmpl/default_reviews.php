...
<div id="reviews-wrapper" itemprop="review" itemscope itemtype="http://schema.org/Review">
    <div class="position-relative clearfix mb-3 reviews-summary">
        ...
        <a class="modal modal-block d-block position-absolute right-0 top-0 m-3" rel="{handler: 'iframe', size: {x: 600, y: 450}}" href='/index.php?option=com_jomdirectory&view=product&layout=chart&id=<?php echo JRequest::getVar('id'); ?>&alias=<?php echo $this->item->alias; ?>&tmpl=component'><i class="mdi mdi-chart-pie mdi-36px"></i>
        </a>
    </div>
    <hr/>
                ...
                <div class="row mb-3">
                    <div class="col-md-3 text-center" itemprop="reviewRating" itemscope itemtype="http://schema.org/Rating">
                        ...
                        <div class="reviews-score hasTip" <?php if (isset($this->item->reviews->rates)): ?>title="<?php foreach ($review->rates as $k => $el): ?><?php echo JText::_($k); ?>: <?php echo (int)$el; ?><br /><?php endforeach; ?>"<?php endif; ?>>
                            <input value="<?php echo round($review->rate * 2) / 2; ?>" type="text" readonly class="dial" data-width="80" data-height="80" data-displayPrevious="true" data-fgColor="#a88e4b" data-thickness=".2" autocomplete="off">
                        </div>
	...
	<?php else: ?>
        <form id="ce-reviews-form" name="ce-reviews-form" method="post" action="<?php echo JRoute::_('index.php?option=com_jomdirectory&task=form.productReviews') ?>" class="form-validate">
