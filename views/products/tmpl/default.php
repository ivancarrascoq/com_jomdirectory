...
$saved = $app->getUserState('com_jomdirectory.products.list.save');
...
$enableMapListing = $this->params->get('product_list_map', 0);
if ($enableMapListing) {
	...
	$runScript .= "Comdev.maps.set({container: 'jd-itemsMap'});";
	$runScript .= "Comdev.maps.initList('" . $this->itemsMap . "');";
}
$runScript .= "}); ";
...
<div id="jomdirectory-products" class="jomdirectory clearfix bootstrap">
	...
	<?php if ($countItems == 0): ?>
		<?php if ($saved): ?>
			<?php echo JHTML::_('link', JRoute::_('index.php?option=com_jomdirectory&task=product.viewUnSaved'), JText::_('COM_JOMDIRECTORY_UNSAVED'), 'class="btn btn-primary"') ?>
		<?php endif; ?>
        <div class="alert alert-primary mt-5"><i class="fa fa-info"></i> <span class=""><?php echo JText::_('COM_JOMDIRECTORY_NO_ITEMS'); ?></span></div>
		<?php return; endif; ?>

	<?php if ($this->params->get('product_list_map')): ?>
        <div class="my-3">
        ...
        <div class="clearfix">
            ...
            <div class="float-right mt-2 mt-md-0">
				<?php if ($this->params->get('product_save')): ?>
					<?php if ($saved): ?>
						<?php echo JHTML::_('link', JRoute::_('index.php?option=com_jomdirectory&task=product.viewUnSaved'), JText::_('COM_JOMDIRECTORY_UNSAVED'), 'class="btn btn-primary mt-0"') ?> &nbsp;
						<?php echo JHTML::_('link', JRoute::_('index.php?option=com_jomdirectory&task=product.clearSaved'), JText::_('COM_JOMDIRECTORY_CLEARSAVED'), 'class="btn btn-primary mt-0"') ?> &nbsp;
					<?php else: ?>
						<?php echo JHTML::_('link', JRoute::_('index.php?option=com_jomdirectory&task=product.viewSaved'), "<i class=\"fa fa-heart\"></i> " . JText::_('COM_JOMDIRECTORY_SAVED') . " <span class=\"badge-cd badge-danger px-2\" id=\"jc-favorite-count\">" . JomcomdevHelperRemember::numFavorite('com_jomdirectory_products') . "</span>", 'class="btn btn-primary mr-1 mt-0"') ?>
						<?php if ($this->params->get('product_compare')): ?>
							<?php echo JHTML::_('link', JRoute::_('index.php?option=com_jomdirectory&task=product.viewCompared'), "<i class=\"fa fa-copy\"></i> " . JText::_('COM_JOMDIRECTORY_PRODUCT_COMPARE') . " <span class=\"badge-cd badge-danger px-2\" id=\"jc-compare-count\">" . JomcomdevHelperRemember::numFavorite('com_jomdirectory_products_compare') . "</span>", 'class="btn btn-secondary mt-0"') ?>
						<?php endif; ?>
					<?php endif; ?>
				<?php endif; ?>
            </div>
    ...
    <div class="row">
		...
		<?php foreach ($this->items as $key => $item):
            ...
            <div class="col-md-<?php echo $column_size; ?> mb-3 d-flex">
                <div class="card">
                    <div class="position-relative view overlay">
                        ...
                        <div class="mask rgba-black-light d-flex">
                            <div class="mx-auto align-self-center">
                                <a class="btn btn-danger btn-sm m-0 px-3" href="<?php echo $link ?>" class="z-index-1"><?php echo JText::_('COM_JOMDIRECTORY_DETAILS'); ?> <i class="fa fa-chevron-right ml-1"></i></a>
                            </div>
                        </div>
                    ...
                    <div class="card-footer p-2 pb-3 mt-3 clearfix">
						<?php if ($this->params->get('cart')): ?>
                            <a data-property="add-to-cart" data-object="{id: <?= $item->id ?>, title: '<?= htmlspecialchars($item->title) ?>', price: <?= $item->price_int ?>, image: '<?= $images ?>'}" class="btn btn-info btn-sm m-0">
                                <i class="fa fa-shopping-cart mr-1"></i> <?php echo JText::_('COM_JOMDIRECTORY_CART_ADD'); ?></a>
						<?php endif; ?>
                        <div id="jd-item-box-toolbar-comp<?php echo $item->id ?>" class="float-right ml-1">
							<?php if ($this->params->get('product_compare')): ?>
								...
								<?php else: ?>
                                    <span class="btn btn-sm btn-secondary px-2 mx-1 my-0 disabled"><i class="fa fa-copy"></i></span>
								<?php endif; ?>
							<?php endif; ?>
                        </div>
