        ...
        <form action="" method="post" name="adminForm" id="adminForm">
            <table class="table table-striped">
                ...
                <tr>
					...
					<?php endif; ?>
                    <th><?php echo JText::_('COM_JOMDIRECTORY_PRODUCT_QUANTITY') ?></th>
                </tr>
                </thead>
                <tbody>
				<?php $i = 0;
				foreach ($this->listings as $row): ?>
                    <tr>
                        <td>
							...
							<?php echo JHtml::_('jgrid.published', $row->published, $i, 'admin_products.', 1, 'cb', $row->date_publish, $row->date_publish_down); ?>
							<?php echo JHtml::_('frontadmin.featured', $row->featured, $i, true, 'admin_products'); ?>
                        </td>
                        <td>
							<?php $link = JRoute::_('?option=com_jomdirectory&task=admin_addproduct.edit&id=' . $row->id); ?>
                            <h4 class="my-1"><?php echo JHTML::_('link', $link, $row->title); ?></h4>
							<?php if (isset($row->company_id) && $row->company_id): ?>
								<?php $link = JRoute::_('?option=com_jomdirectory&task=admin_additem.edit&id=' . $row->company_id); ?>
                                <span><i class="fa fa-link"></i> <?php echo JHTML::_('link', $link, $row->companyTitle); ?></span>
							<?php endif; ?>
                        </td>
						<?php if ($this->params->get('product_price')): ?>
                            <td class="text-right align-middle">
                                <span class="font-weight-bold">
									<?php echo $this->escape($row->price) . $this->params->get('adm_currency', 'EU'); ?>
                                    (VAT <?php if ($row->tax): echo $row->tax;
	                                else: echo $this->params->get('currency_brutto', 23);
	                                endif; ?>%)
                                </span>
                            </td>
						<?php endif; ?>
                        <td class="text-center">
                            <span class="btn btn-sm btn-secondary disabled"><?php echo $this->escape($row->quantity); ?></span>
                        </td>
                    </tr>
					<?php $i++; endforeach; ?>
                </tbody>
