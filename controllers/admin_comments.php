 ...
 */
class JomdirectoryControllerAdmin_comments extends JControllerLegacy
{
	...
	function unpublish()
	{
		...
		if (count($cid)) {
			$cids = implode(',', $cid);
			$query = 'UPDATE #__cddir_reviews SET approved ="0" WHERE id IN ( ' . $cids . ' ) and extension="com_jomdirectory"';
			$db->setQuery($query);
			if (!$db->query()) {
				echo "<script> alert('" . $db->getErrorMsg(true) . "'); window.history.go(-1); </script>\n";
			}
