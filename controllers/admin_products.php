 ...
 */
class JomdirectoryControllerAdmin_products extends JControllerLegacy
{
	protected $user;
	...
	{
		$db = JFactory::getDBO();
		$query = "SELECT  COUNT(*) FROM #__cddir_products WHERE users_id={$this->user->id} and published=1";
		$db->setQuery($query);
		...
		if ($limits && $listings_nr > $limits->listings_nr) JError::raiseWarning(100, JText::_("COM_JOMDIRECTORY_NOT_ENOUGH_LISTINGS_IN_PLAN")); else if (count($cid)) {
			$cids = implode(',', $cid);
			$query = 'UPDATE #__cddir_products SET published ="1" WHERE id IN ( ' . $cids . ' ) AND users_id=' . (int)$this->user->id;
			$db->setQuery($query);
			if (!$db->query()) {
	...
	function unpublish()
	{
		...
		if (count($cid)) {
			$cids = implode(',', $cid);
			$query = 'UPDATE #__cddir_products SET published ="0" WHERE id IN ( ' . $cids . ' ) AND users_id=' . (int)$this->user->id;
			$db->setQuery($query);
			if (!$db->query()) {
				echo "<script> alert('" . $db->getErrorMsg(true) . "'); window.history.go(-1); </script>\n";
			}
	...
	function delete()
	{
		...
		if (count($cid)) {
			$cids = implode(',', $cid);
			$query = 'DELETE FROM #__cddir_products WHERE id IN ( ' . $cids . ' ) AND users_id=' . (int)$this->user->id;
			$db->setQuery($query);
			if (!$db->query()) {
				echo "<script> alert('" . $db->getErrorMsg(true) . "'); window.history.go(-1); </script>\n";
			}
	...
	{
		$db = JFactory::getDBO();
		$query = "SELECT  COUNT(*) FROM #__cddir_products WHERE users_id={$this->user->id} and featured=1";
		$db->setQuery($query);
		$listings_nr = $db->loadResult();
		$cid = JRequest::getVar('cid', array(), '', 'array');
		JArrayHelper::toInteger($cid);
		...
		if ($limits && $listings_nr > $limits->premium_nr) JError::raiseWarning(100, JText::_("COM_JOMDIRECTORY_NOT_ENOUGH_PREMIUM_IN_PLAN")); else if (count($cid)) {
			$cids = implode(',', $cid);
			$query = 'UPDATE #__cddir_products SET featured ="1" WHERE id IN ( ' . $cids . ' ) AND users_id=' . (int)$this->user->id;
			$db->setQuery($query);
			if (!$db->query()) {
				echo "<script> alert('" . $db->getErrorMsg(true) . "'); window.history.go(-1); </script>\n";
			}
	...
	public function unfeatured()
	{
		...
		if (count($cid)) {
			$cids = implode(',', $cid);
			$query = 'UPDATE #__cddir_products SET featured ="0" WHERE id IN ( ' . $cids . ' ) AND users_id=' . (int)$this->user->id;
			$db->setQuery($query);
			if (!$db->query()) {
				echo "<script> alert('" . $db->getErrorMsg(true) . "'); window.history.go(-1); </script>\n";
			}
	...
	function add()
	{
		...
		$app->redirect('index.php?option=com_jomdirectory&view=admin_addproduct');
	}
}
